...
def export_to_pdf():
    """
    Формирует данные для экспорта и генерирует PDF-файл с использованием упрощенной реализации.
    ...
    # Используем значения из сессии как дефолт, если данные отсутствуют
    default_width = st.session_state.get("cached_width") or 3.0
    default_length = st.session_state.get("cached_length") or 4.0
    
    # Формируем данные для PDF
    pdf_data = {
        "pergola_type": options.get("pergola_type", "B500"),
        ...
        "has_lighting": "led_lighting" in options,
        "has_sensors": "sensors" in options,
    ...
    try:
        # Используем наш новый модуль для генерации PDF
        from basic_pdf_export import create_pergola_pdf
        
        # Генерируем PDF
        print("Генерация PDF через модуль basic_pdf_export...")
        pdf_file_path = create_pergola_pdf(pdf_data)
        
        if pdf_file_path:
            print(f"PDF файл успешно создан: {pdf_file_path}")
            logging.info(f"PDF файл успешно создан: {pdf_file_path}")
