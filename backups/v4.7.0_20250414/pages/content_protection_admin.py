...
try:
    ...
    st.error("Модуль content_protection.py или components/admin_auth.py не найден. Убедитесь, что он существует в проекте.")
    st.stop()
...
def main():
    ...
    if not is_authenticated:
        st.warning("⚠️ Для доступа к управлению защитой контента требуется авторизация")
    ...
    # Эти блоки будут отображаться только если пользователь аутентифицирован
    if is_authenticated:
    ...
    # Все функции отображаются только если пользователь аутентифицирован
    if is_authenticated:
        cols = st.columns(2)
