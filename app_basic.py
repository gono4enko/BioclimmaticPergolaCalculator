...
def render_results(results):
    ...
    if not st.session_state.get('description_shown', False):
        ...
        if pergola_type in ["B500NEW", "B700NEW", "B600"]:
            ...
            with st.container():
                st.write("## Биоклиматическая пергола: когда установка начинается с проектирования")
                st.write("*Тонкости монтажа, о которых не расскажет обычный продавец*")
                
                st.write("### Интеграция перголы в существующую архитектуру: нет ограничений")
                st.write("Биоклиматическая пергола «растёт» вместе с вашими потребностями и может быть смонтирована четырьмя разными способами:")
                
                st.write("1. **Отдельно стоящая конструкция** идеальна для открытых пространств: летних площадок и террас. Даже без фундамента — на специальных утяжелителях.")
                st.write("2. **Примыкание к стене** экономит пространство и создаёт плавный переход между домом и садом. Даже для стеклянного фасада найдутся решения.")
                st.write("3. **Встраивание в проем** создаёт крышу для балконов верхних этажей или защищает окна от перегрева. Можно интегрировать даже в скошенную кровлю.")
                st.write("4. **Навес в труднодоступной зоне** со специальными решениями высотного монтажа. Бригада промышленных альпинистов справится с любой задачей.")
                
                st.write("### Климатические системы: пять уровней комфорта")
                st.write("Пергола уже давно не просто «крыша над головой». Это модульная система с адаптивными опциями:")
                
                st.write("* **Базовая система:** Ламели, которыми вы управляете по таймеру, датчикам или вручную.")
                st.write("* **Дождевая защита:** Встроенные лотки отводят до 80 л/м² осадков, а сама система автоматически закрывается при первых каплях.")
                st.write("* **Тепловой контроль:** Светодиодные инфракрасные обогреватели на магнитных креплениях с регулировкой до +50°C.")
                st.write("* **Ветрозащита:** ZIP-экраны с тканью Ferrari задерживают до 92% солнечных лучей и выдерживают порывы до 60 км/ч.")
                st.write("* **Управление микроклиматом:** Ламели создают эффект Вентури, когда нужно проветрить, не допуская сквозняков.")
                
                st.write("### Автоматика: технологии, которые работают за вас")
                st.write("Для управления перголой не нужны инструкции толщиной с роман:")
                
                st.write("* Единый пульт Somfy или Simu управляет всеми элементами: от ламелей до света.")
                st.write("* Датчики дождя, ветра, снега и температуры автоматически адаптируют пергольную систему к погоде.")
                st.write("* Умный дом: интеграция с Apple HomeKit, Google Home, Яндекс и даже с Алисой.")
                st.write("* Европейская надежность: Автоматика Bansbach и Somfy (до 50 000 циклов!) работают даже в экстремальных условиях.")
                
                st.write("**Вы готовы выйти за рамки стандартов? Биоклиматическая пергола — это не «навес», а ваш личный климатический контролер. Оставьте заявку, и наши инженеры создадут проект, где каждая деталь будет работать на ваш комфорт.**")
            ...
            if pergola_type in ["B500NEW", "B700NEW"]:
                with st.container():
                    st.write("## Инженерный взгляд на биоклиматические перголы: как ламели выдерживают снег, ливни и сохраняют тепло")
                    st.write("*Технические секреты, которые делают ваш комфорт непогрешимым*")
                    
                    st.write("### Прогиб ламелей: где точность встречает надежность")
                    st.write("Ламели — это «скелет» перголы, и их прочность определяет, как конструкция поведет себя под нагрузками. Мы протестировали каждую модель в экстремальных условиях. Вот что важно знать:")
                    
                    st.write("**Ламель 250×53 NEW**")
                    st.write("* Масса: 4,684 кг/м | Шаг: 250 мм")
                    st.write("* Прогиб под нагрузкой:")
                    st.write("  * Рабочая (масса ламели + 50%): 12 мм при ширине 4,5 м.")
                    st.write("  * Снеговая (50 кг/м²): 30 мм при ширине 4,5 м.")
                    st.write("* Макс. снеговая нагрузка: 50 кг/м² (эквивалент 40-50 см свежего снега).")
                    
                    st.write("**Ламель 200×56 NEW**")
                    st.write("* Масса: 4,375 кг/м | Шаг: 200 мм")
                    st.write("* Прогиб под нагрузкой:")
                    st.write("  * Рабочая: 10 мм при ширине 5,0 м.")
                    st.write("  * Снеговая: 22 мм при ширине 5,0 м.")
                    st.write("* Макс. снеговая нагрузка: 70 кг/м² (до 60 см снега).")
                    
                    st.write("Почему это важно? Минимальный прогиб сохраняет геометрию крыши даже в экстремальных условиях. Для сравнения: у конкурентов аналогичные ламели прогибаются на 50-70 мм под теми же нагрузками.")
                    
                    st.write("### Герметичность: когда внутри сухо, даже если снаружи потоп")
                    st.write("Алюминиевые ламели перголы — это не просто «крыша», а герметичный кокон. Вот как мы этого добиваемся:")
                    
                    st.write("1. **Двойное уплотнение EPDM:**")
                    st.write("   * Уплотнители между ламелями и в лотках выдерживают давление водяного столба до 600 Па (ливень интенсивностью 100 мм/час).")
                    st.write("   * Для сравнения: стандартные системы держат только 300-400 Па.")
                    st.write("2. **Наклон ламелей + дренажные каналы:** Вода стекает в лотки, не задерживаясь на поверхности. Даже при сильном ветре капли не просачиваются через стыки.")
                    st.write("3. **Тест на герметичность:** Мы заливаем крышу перголы водой под давлением 600 Па в течение 1 часа. Результат: 0 протечек.")
                    
                    st.write("### Теплоизоляция: почему под перголой тепло зимой и прохладно летом")
                    st.write("* **Терморазрыв в профиле:** Алюминиевые ламели с изолирующими вставками из нейлона снижают теплопотери на 40%.")
                    st.write("* **Эффект «воздушной подушки»:** При закрытых ламелях между ними образуется прослойка воздуха, которая работает как естественный изолятор.")
                    st.write("* **Температурный комфорт:**")
                    st.write("  * Зимой: под перголой на 5-7°C теплее, чем снаружи.")
                    st.write("  * Летом: на 10-12°C прохладнее благодаря тени и вентиляции.")
                    
                    st.write("### Сравнительная таблица: какая ламель подходит вам?")
                    
                    data = {
                        "Параметр": ["Снеговая нагрузка", "Прогиб под снегом", "Шаг ламелей", "Ветровая стойкость", "Идеальный сценарий"],
                        "Ламель 250×53 NEW": ["50 кг/м²", "30 мм (4,5 м)", "250 мм", "До 25 м/с", "Умеренный климат"],
                        "Ламель 200×56 NEW": ["70 кг/м²", "22 мм (5,0 м)", "200 мм (плотнее защита)", "До 30 м/с", "Северные регионы"]
                    }
                    
                    df = pd.DataFrame(data)
                    st.table(df)
                    
                    st.write("### Почему это выгодно?")
                    st.write("* **Экономия на отоплении/кондиционировании:** Терморазрыв и герметичность снижают затраты на энергию.")
                    st.write("* **Гарантия 10 лет:** На ламели и уплотнители — потому что мы уверены в каждом миллиметре.")
                    st.write("* **Адаптивность:** Комбинируйте ламели с остеклением или ZIP-экранами для полного контроля над климатом.")
                    
                    st.write("**Финал: Биоклиматическая пергола — это не «навес», а инженерная система, которая считает каждую каплю дождя и снежинку. Выбирайте ламели, которые работают как швейцарские часы — тихо, точно, без компромиссов.**")
