...
def render_results(results):
    ...
    if not st.session_state.get('description_shown', False):
        ...
        if pergola_type == "B500NEW":
            ...
            try:
                st.image("attached_assets/IMG_1002.jpeg", caption="Пергола B700 со сдвижными ламелями", use_container_width=True)
            except Exception as e:
                try:
            ...
            # Добавляем изображение модульной системы
            try:
                st.image("attached_assets/Модульная система пергол.jpeg", caption="Принцип модульной системы пергол", use_container_width=True)
            except Exception as e:
                st.warning(f"Не удалось загрузить изображение модульной системы: {e}")
