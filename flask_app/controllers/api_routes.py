...
@bp.route('/video-thumb/<video_id>')
def video_thumbnail(video_id):
    if not all(c in '0123456789abcdef' for c in video_id) or len(video_id) != 32:
        return '', 404

    cache_dir = os.path.join('data', 'video_thumbs')
    cached_path = os.path.join(cache_dir, video_id + '.jpg')
    ...
    try:
        if video_id in _thumb_cache:
            thumb_url = _thumb_cache[video_id]
        else:
            ...
            if api_resp.status_code != 200:
                return '', 404
            thumb_url = api_resp.json().get('thumbnail_url', '')
            _thumb_cache[video_id] = thumb_url

        if not thumb_url:
            return '', 404

        img_resp = _requests.get(thumb_url, timeout=5)
