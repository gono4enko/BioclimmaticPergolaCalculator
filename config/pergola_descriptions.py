...
PERGOLA_IMAGE_CAPTIONS = {
    ...
    "TECHNICAL": "Технические характеристики ламелей для пергол"
}
...
# Изображения для вариантов установки
PERGOLA_IMAGES["INSTALLATION"] = [
    "attached_assets/Instal and design Sistem.png"
]

# Подпись к изображениям вариантов установки
PERGOLA_IMAGE_CAPTIONS["INSTALLATION"] = "Варианты установки биоклиматических пергол"
...
TECHNICAL_ENGINEERING_DESCRIPTION = """
<div style='padding: 0 20px;'>
...
<div style='margin-bottom: 20px;'>
<h4 style='font-size: 1.1rem; margin-top: 15px;'>Прогиб ламелей: где точность встречает надежность</h4>
...
<p style='margin-bottom: 15px;'>
<strong>Почему это важно?</strong> Минимальный прогиб сохраняет геометрию крыши даже в экстремальных условиях. Для сравнения: у конкурентов аналогичные ламели прогибаются на 50-70 мм под теми же нагрузками.
</p>
</div>

<div style='margin-bottom: 20px;'>
<h4 style='font-size: 1.1rem; margin-top: 15px;'>Герметичность: когда внутри сухо, даже если снаружи потоп</h4>
<p style='margin-bottom: 10px;'>
Алюминиевые ламели перголы — это не просто «крыша», а герметичный кокон. Вот как мы этого добиваемся:
</p>
...
<div style='margin-bottom: 20px;'>
<h4 style='font-size: 1.1rem; margin-top: 15px;'>Теплоизоляция: почему под перголой тепло зимой и прохладно летом</h4>
<ul style='margin-bottom: 15px; padding-left: 20px;'>
...
<div style='margin-bottom: 15px;'>
<h4 style='font-size: 1.1rem; margin-top: 15px;'>Почему это выгодно?</h4>
<ul style='margin-bottom: 15px; padding-left: 20px;'>
...
def get_lamella_engineering_description(pergola_type=None):
    ...
    Args:
        pergola_type (str, optional): Тип перголы (B500NEW, B700NEW)
        
    Returns:
        str: HTML-описание технических характеристик ламелей
    """
    # Для B500NEW и B700NEW возвращаем подробное техническое описание ламелей
    if pergola_type in ["B500NEW", "B700NEW"]:
        return TECHNICAL_ENGINEERING_DESCRIPTION
    
    # Для остальных типов возвращаем базовое описание
